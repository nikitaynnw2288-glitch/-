...
Модули: email, username, phone, telegram, metadata, domain, text, geo, geo_ai, hibp, ip + Google Drive базы.
"""
...
def init_db():
    with db() as c:
        ...
        CREATE TABLE IF NOT EXISTS bases (
            ...
            name TEXT UNIQUE, gdrive_id TEXT, size INTEGER,
            rows INTEGER DEFAULT 0, fields TEXT, format TEXT,
...
class GDrive:
    ...
    def enabled(self) -> bool:
        return bool(settings.GDRIVE_SA_JSON and settings.GDRIVE_FOLDER_ID)

    def _load_sa(self):
        if self._sa is None:
            try:
                self._sa = json.loads(settings.GDRIVE_SA_JSON)
            except Exception:
                self._sa = {}
        return self._sa
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=20) as c:
                ...
                if r.status_code != 200:
                    log("gdrive_auth", None, r.text[:300], level="error")
                    return None
                data = r.json()
                self._token = data["access_token"]
                self._token_exp = _time.time() + data.get("expires_in", 3600)
                return self._token
        except Exception as e:
            log("gdrive_auth", None, str(e), level="error")
            return None
    ...
    async def list_files(self, folder_id: str = None) -> list[dict]:
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as c:
                r = await c.get(f"{self.API}/files", params={
                    "q": f"'{fid}' in parents and trashed=false",
                    "fields": "files(id,name,size,mimeType,modifiedTime)",
                    "pageSize": 1000,
                }, headers={"Authorization": f"Bearer {token}"})
                if r.status_code != 200:
                    log("gdrive_list", None, r.text[:300], level="error")
                    return []
                return r.json().get("files", [])
        except Exception as e:
            log("gdrive_list", None, str(e), level="error")
            return []

    async def download(self, file_id: str, dest_path: Path) -> bool:
        token = await self._get_token()
        if not token:
            return False
        try:
            ...
            log("gdrive_download", None, str(e), level="error")
            return False
...
def guess_field(value: str) -> str:
    v = value.strip()
    if not v:
        return "other"
    if FIELD_PATTERNS["phone"].match(v.replace(" ", "").replace("-", "")):
        return "phone"
    if FIELD_PATTERNS["email"].match(v):
        return "email"
    if FIELD_PATTERNS["date"].match(v):
        return "date"
    if FIELD_PATTERNS["ip"].match(v):
        return "ip"
    if FIELD_PATTERNS["fio"].match(v):
        return "fio"
    return "other"
...
def parse_line(line: str, fmt: str) -> list:
    if fmt == "sql":
...
class GeoAIModule(BaseModule):
    ...
    @staticmethod
    async def analyze(image_bytes: bytes, filename: str = "photo.jpg"):
...
class HIBPModule(BaseModule):
    ...
    def _check_password(password):
        ...
        try:
            r = httpx.get(url, headers={"Add-Padding": "true"}, timeout=10)
            if r.status_code != 200:
                return 0
    ...
    async def _check_breaches(self, email, client):
        ...
        try:
            ...
            log("hibp_error", None, str(e), level="error")
        return []
        ...
        if settings.HIBP_KEY:
            ...
            if breaches:
                ...
                for b in breaches[:10]:
                    lines.append(
                        f"• <b>{b.get('Title', '?')}</b> ({b.get('Domain', '?')})\n"
                        f"  Дата: {b.get('BreachDate', '?')}\n"
                        f"  Утекло: {', '.join(b.get('DataClasses', [])[:5])}"
                    )
                findings.append(Finding(
                    "HIBP утечки", "hibp", target,
                    {"Найдено": str(len(breaches)), "Список": "\n".join(lines)},
                    0.9,
                ))
            else:
                findings.append(Finding(
                    "HIBP утечки", "hibp", target,
                    {"Статус": "✅ не найден в утечках"}, 0.7,
                ))
        else:
            findings.append(Finding(
                "HIBP утечки", "hibp", target,
                {"Статус": "⚠️ HIBP_KEY не задан"}, 0.3,
            ))

        pw_count = self._check_password(target)
        if pw_count > 0:
            findings.append(Finding(
                "Pwned Passwords", "hibp", target,
                {"Статус": f"⚠️ email встречается как пароль {pw_count} раз"}, 0.8,
            ))
...
class IPModule(BaseModule):
    ...
    async def run(cls, target, client):
        ...
        if not cls.IP_RE.match(ip):
            return ModuleResult(cls.name, target, False,
                                error="Неверный формат IP (нужен IPv4)")
        ...
        try:
            ...
            return ModuleResult(cls.name, target, True,
                                [Finding("IP-инфо", "ip", ip, info, 0.9)],
                                duration_ms=int((_time.time() - t0) * 1000))
        except Exception as e:
            return ModuleResult(cls.name, target, False, error=str(e),
                                duration_ms=int((_time.time() - t0) * 1000))
...
class BasesModule:
    """Работа с базами на Google Drive: sync → index → search."""

    @staticmethod
    def list_bases() -> list:
        with db() as c:
            return [dict(r) for r in c.execute(
                "SELECT id,name,gdrive_id,size,rows,fields,format,indexed_at,status "
                "FROM bases ORDER BY name"
            ).fetchall()]
    ...
    async def sync_from_gdrive() -> dict:
        ...
        with db() as c:
            for f in files:
                ...
                if not row:
                    c.execute("INSERT INTO bases (name,gdrive_id,size,format,status) "
                              "VALUES (?,?,?,?,?)", (name, fid, size, fmt, "pending"))
                    added += 1
                else:
    ...
    async def index_base(base_id: int, progress_cb=None) -> dict:
        ...
        try:
                ...
                for raw in f:
                    ...
                    if fmt == "sql":
                        ...
                        if rows_total == 0 and (";" in line or "," in line or "|" in line):
                            low = line.lower()
                            if any(k in low for k in ["фио", "fio", "тел", "phone",
                                                       "email", "почт", "имя"]):
                                rows_total += 1
                                continue
                        values = [x.strip().strip('"') for x in
                                  line.split(detect_delimiter(line))]
                    else:
                    ...
                    if len(batch_rows) >= 500:
                        with db() as c:
                            c.executemany("INSERT INTO base_rows (base_id,row_num,data) "
                                          "VALUES (?,?,?)", batch_rows)
                            last_id = c.execute(
                                "SELECT MAX(id) FROM base_rows WHERE base_id=?",
                                (base_id,)).fetchone()[0]
                            ...
                            for i, br in enumerate(batch_rows):
                                ...
                                for v in json.loads(br[2]):
                                    ...
                                    if fld != "other" and len(str(v)) >= 3:
                                        batch_idx.append(
                                            (base_id, fld, str(v).strip(), row_id))
                            c.executemany(
                                "INSERT INTO base_index (base_id,field,value,row_id) "
                                "VALUES (?,?,?,?)", batch_idx)
                        batch_rows = []
                        batch_idx = []
            ...
            with db() as c:
                c.execute("UPDATE bases SET rows=?, fields=?, indexed_at=?, status='ready' "
                          "WHERE id=?", (rows_total, fields_str, int(_time.time()), base_id))
    ...
    @staticmethod
    def search(base_id: int, query: str, field: str = None, limit: int = 50) -> list:
        q = query.strip().lower()
        if not q:
            return []
        with db() as c:
    ...
    @staticmethod
    def search_all(query: str, field: str = None, limit: int = 30) -> list:
        q = query.strip().lower()
        if not q:
            return []
        results = []
...
def fmt_base_info(base):
    ...
    if base.get("fields"):
        lines.append(f"🏷 <b>Поля:</b> {base['fields'][:200]}")
...
async def start_ref(m: Message, command: CommandObject):
    ...
    text = (
        ...
        "▫️ <b>Email утечки</b> — HIBP\n"
        "▫️ <b>IP</b> — геолокация и провайдер\n"
        ...
        "▫️ <b>🗄 Базы</b> — поиск по загруженным базам\n\n"
        "Просто напиши цель 👇"
    )
...
async def help_cmd(m: Message):
    await m.answer(
        ...
        "Команды: /scan /ref /me /bases /admin",
        reply_markup=main_kb(m.from_user.id))
...
async def kb_bases(m: Message):
    if not gdrive.enabled:
        await m.answer("⚠️ Google Drive не настроен.\nДобавь GDRIVE_SA_JSON и GDRIVE_FOLDER_ID в Railway Variables.")
        return
    text = fmt_bases_menu()
...
async def cb_base_info(cb: CallbackQuery):
    ...
    if not base:
        return await cb.answer("Не найдена", show_alert=True)
...
async def cb_base_index(cb: CallbackQuery):
    ...
    if res.get("ok"):
        ...
        await msg.edit_text(f"❌ Ошибка: <code>{res.get('error')}</code>",
                            reply_markup=base_info_kb(bid))
...
async def process_search_query(m: Message, state: FSMContext):
    ...
    log("base_search", m.from_user.id, f"query={query[:50]} found={len(results)}")
